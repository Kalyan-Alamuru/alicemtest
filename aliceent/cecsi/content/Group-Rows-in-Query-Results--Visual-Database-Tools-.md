...
![Query results: average price grouped by publisher](../content/media/dv3w9e1.gif "dv3w9e1")  
...
You cannot display values from individual rows. For example, if you group only by publisher, you cannot also display individual titles in the query. Therefore, if you add columns to the query output, the [Query and View Designer](../content/Query-and-View-Designer-Tools--Visual-Database-Tools-.md) automatically adds them to the GROUP BY clause of the statement in the [SQL pane](../content/SQL-Pane--Visual-Database-Tools-.md). If you want a column to be aggregated instead, you can specify an aggregate function for that column.  
...
![Query results: price grouped by publisher and type](../content/media/dv3w9e2.gif "dv3w9e2")  
...
5.  In the **Group By** grid cell for the column that is going to be aggregated, select the appropriate aggregate function.  
    ...
    The Query and View Designer automatically assigns a column alias to the column you are summarizing. You can replace this automatically generated alias with a more meaningful one. For more details, see [Create Column Aliases (Visual Database Tools)](../content/Create-Column-Aliases--Visual-Database-Tools-.md).  
  
    ![Adding a column alias to the query result set](../content/media/dv3w9e3.gif "dv3w9e3")  
