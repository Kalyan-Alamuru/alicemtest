...
> [!CAUTION]  
>  For information on the statement states that **SQLSetPos** can be called in and what it needs to do for compatibility with ODBC 2*.x* applications, see [Block Cursors, Scrollable Cursors, and Backward Compatibility](../content/Block-Cursors--Scrollable-Cursors--and-Backward-Compatibility.md).  
...
-   If the application specifies SQL_CONCUR_READ_ONLY for the SQL_ATTR_CONCURRENCY statement attribute, the driver rejects any update or delete operation.  
 ...
 For more information about the SQL_ATTR_CONCURRENCY statement attribute, see [SQLSetStmtAttr](../content/SQLSetStmtAttr-Function.md).  
...
-   The row operation array (as pointed to by the SQL_DESC_ARRAY_STATUS_PTR field in the ARD and the SQL_ATTR_ROW_OPERATION_ARRAY statement attribute) contains a value for each row in the rowset that indicates whether a call to **SQLSetPos** for a bulk operation is ignored or performed. Each element in the array is set to either SQL_ROW_PROCEED (the default) or SQL_ROW_IGNORE. This array is pointed to by the SQL_ATTR_ROW_OPERATION_PTR statement attribute.  
 ...
 For information about the row status array, see [SQLFetch](../content/SQLFetch-Function.md). For information about the row operation array, see "Ignoring a Row in a Bulk Operation," later in this section.  
...
4.  Call **SQLFetch** or **SQLFetchScroll** to retrieve the data.  
 ...
 For more information about using **SQLSetPos**, see [Updating Data with SQLSetPos](../content/Updating-Data-with-SQLSetPos.md).  
...
## Ignoring a Column in a Bulk Operation  
 To avoid unnecessary processing diagnostics generated by attempted updates to one or more read-only columns, an application can set the value in the bound length/indicator buffer to SQL_COLUMN_IGNORE. For more information, see [SQLBindCol](../content/SQLBindCol-Function.md).  
...
```  
 ...
 For more examples, see [Positioned Update and Delete Statements](../content/Positioned-Update-and-Delete-Statements.md) and [Updating Rows in the Rowset with SQLSetPos](../content/Updating-Rows-in-the-Rowset-with-SQLSetPos.md).  
...
|Performing bulk operations that do not relate to the block cursor position|[SQLBulkOperations Function](../content/SQLBulkOperations-Function.md)|  
...
|Getting a single field of a descriptor|[SQLGetDescField Function](../content/SQLGetDescField-Function.md)|  
|Getting multiple fields of a descriptor|[SQLGetDescRec Function](../content/SQLGetDescRec-Function.md)|  
|Setting a single field of a descriptor|[SQLSetDescField Function](../content/SQLSetDescField-Function.md)|  
|Setting multiple fields of a descriptor|[SQLSetDescRec Function](../content/SQLSetDescRec-Function.md)|  
