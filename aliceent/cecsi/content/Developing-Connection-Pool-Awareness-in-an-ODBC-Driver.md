...
-   SQLCleanupConnectionPoolID  
 ...
 See [ODBC Service Provider Interface (SPI) Reference](../content/ODBC-Service-Provider-Interface--SPI--Reference.md) for more information.  
...
|--------------|-------------------------|  
|[SQLAllocHandle](../content/SQLAllocHandle-Function.md)<br /><br /> [SQLFreeHandle](../content/SQLFreeHandle-Function.md)<br /><br /> [SQLGetDiagField](../content/SQLGetDiagField-Function.md)<br /><br /> [SQLGetDiagRec](../content/SQLGetDiagRec-Function.md)|Support the new handle type: SQL_HANDLE_DBC_INFO_TOKEN (see the description below).|  
|[SQLSetConnectAttr](../content/SQLSetConnectAttr-Function.md)|Support the new set-only connection attribute: SQL_ATTR_DBC_INFO_TOKEN for resetting the connection (see the description below).|  
...
## The Pool ID  
 ...
 The function for getting the pool ID from the driver is [SQLGetPoolID Function](../content/SQLGetPoolID-Function.md).  
...
-   If the mismatched occurred for an attribute that is very time-consuming to reset (the developer of the driver may consider adding this attribute into the set of key attributes, which is used to generate the pool ID).  
 ...
 A score between 0 and 100 is possible, where 0 means do not reuse and 100 means perfectly matched. [SQLRateConnection](../content/SQLRateConnection-Function.md) is the function for rating a connection.  
...
## New ODBC Handle - SQL_HANDLE_DBC_INFO_TOKEN  
 ...
 Whenever it receives a new connection request, the Driver Manager will allocate a handle of type SQL_HANDLE_DBC_INFO_TOKEN for storing connection information, after it confirms that the driver supports connection-pool awareness. When finished using the handle (but before returning some return codes other than SQL_STILL_EXECUTING from [SQLDriverConnect](../content/SQLDriverConnect-Function.md) or [SQLConnect](../content/SQLConnect-Function.md)), the Driver Manager will free the handle. Therefore, the handle is created after the SQLAllocHandle call, and destroyed after the SQLFreeHandle call. The Driver Manager guarantees the handle will be freed before freeing its associated HENV (when [SQLDriverConnect](../content/SQLDriverConnect-Function.md) or [SQLConnect](../content/SQLConnect-Function.md) returns an error).  
  
 The driver should modify the following functions to accept the new handle type SQL_HANDLE_DBC_INFO_TOKEN:  
  
1.  [SQLAllocHandle](../content/SQLAllocHandle-Function.md)  
  
2.  [SQLFreeHandle](../content/SQLFreeHandle-Function.md)  
  
3.  [SQLGetDiagField](../content/SQLGetDiagField-Function.md)  
  
4.  [SQLGetDiagRec](../content/SQLGetDiagRec-Function.md)  
...
-   For driver that is expensive to enlist and unenlist (see the discussion of SQL_DTC_TRANSITION_COST in [SQLConnect](../content/SQLConnect-Function.md)), reusing *hRequest* must not require an extra enlistment or unenlistment.  
...
## Sequence Diagram  
 This sequence diagram shows the basic pooling mechanism described in this topic. It only shows the use of [SQLDriverConnect](../content/SQLDriverConnect-Function.md) but the [SQLConnect](../content/SQLConnect-Function.md) case is similar.  
  
 ![Sequence Diagram](../content/media/odbc_seq_dia.gif "odbc_seq_dia")  
  
## State Diagram  
 This state diagram shows the connection info token object, described in this topic. The diagram only shows [SQLDriverConnect](../content/SQLDriverConnect-Function.md) but the [SQLConnect](../content/SQLConnect-Function.md) case is similar. Since the Driver Manager may need to handle errors at any time, the Driver Manager can call [SQLFreeHandle](../content/SQLFreeHandle-Function.md) for any state.  
  
 ![State Diagram](../content/media/odbc_state_diagram.gif "odbc_state_diagram")  
