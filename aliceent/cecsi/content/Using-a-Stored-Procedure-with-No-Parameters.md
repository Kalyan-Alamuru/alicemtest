...
# Using a Stored Procedure with No Parameters
  The simplest kind of  SQL Server  stored procedure that you can call is one that contains no parameters and returns a single result set. The  Microsoft JDBC Driver for SQL Server  provides the [SQLServerStatement](../content/SQLServerStatement-Class.md) class, which you can use to call this kind of stored procedure and process the data that it returns.  
...
```  
 ...
 In the following example, an open connection to the  [AdventureWorks](http://msftdbprodsamples.codeplex.com/)  sample database is passed in to the function, and the [executeQuery](../content/executeQuery-Method--SQLServerStatement-.md) method is used to call the GetContactFormalNames stored procedure.  
