...
|HY008|Operation canceled|Asynchronous processing was enabled for the *ConnectionHandle*. The function was called, and before it completed execution, the [SQLCancelHandle function](../content/SQLCancelHandle-Function.md) was called on the *ConnectionHandle*, and then the **SQLDriverConnect** function was called again on the *ConnectionHandle*.<br /><br /> Or, the **SQLDriverConnect** function was called, and before it completed execution, **SQLCancelHandle** was called on the *ConnectionHandle* from a different thread in a multithread application.|  
