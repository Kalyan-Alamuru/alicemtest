...
-   A driver written to the Open Group and ISO CLI specifications will work with an ODBC 3*.x* application or a standards-compliant application when it is compiled with the ODBC 3*.x* header files and linked with ODBC 3*.x* libraries, and when the application gains access to the driver through the ODBC 3*.x* Driver Manager. (For more information, see [Standards-Compliant Applications and Drivers](../content/Standards-Compliant-Applications-and-Drivers.md).  
