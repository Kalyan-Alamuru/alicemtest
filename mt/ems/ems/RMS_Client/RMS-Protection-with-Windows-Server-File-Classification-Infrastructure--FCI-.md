...
-   On each file server where you will run File Resource Manager with file classification infrastructure:
    ...
    -   If you want to change the default level of RMS protection (native or generic) for specific file name extensions, you have edited the registry as described in the [File API configuration](https://msdn.microsoft.com/library/dn197834(v=vs.85).aspx) page.
...
1.  Expand the [Windows PowerShell Script](#BKMK_RMSProtection_Script) section in this article, and copy its contents. Paste the contents of the script and  name the file **RMS\-Protect\-FCI.ps1** on your own computer.
