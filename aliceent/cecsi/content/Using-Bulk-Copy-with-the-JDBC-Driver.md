...
>   
>  For more information, see [Transaction and bulk copy operations](../content/Using-Bulk-Copy-with-the-JDBC-Driver.md#BKMK_TransactionBulk)  
...
> [!IMPORTANT]  
>  This sample will not run unless you have created the work tables as described in [Table setup](../content/Using-Bulk-Copy-with-the-JDBC-Driver.md#BKMK_TableSetup). This code is provided to demonstrate the syntax for using SQLServerBulkCopy only. If the source and destination tables are located in the same SQL Server instance, it is easier and faster to use a Transact-SQL INSERT … SELECT statement to copy the data.  
