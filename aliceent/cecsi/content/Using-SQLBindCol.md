...
-   The C data type, address, and byte length of the variable bound to the column. It is an error to specify a C data type to which the SQL data type of the column cannot be converted; this error might not be detected until the result set has been created, so it is returned by **SQLFetch**, not **SQLBindCol**. For a list of supported conversions, see [Converting Data from SQL to C Data Types](../content/Converting-Data-from-SQL-to-C-Data-Types.md) in Appendix D: Data Types. For information about the byte length, see [Data Buffer Length](../content/Data-Buffer-Length.md).  
  
-   The address of a length/indicator buffer. The length/indicator buffer is optional. It is used to return the byte length of binary or character data or return SQL_NULL_DATA if the data is NULL. For more information, see [Using Length/Indicator Values](../content/Using-Length-and-Indicator-Values.md).  
